...
for(let card in items) {
    if(items[card].orderInfo.reviews > 50) {
        var result = "Above avarage";
    } else {
       var result = "Less than average";
    }
    
    var idProd = items[card].id;
     productCard += ` 
    
        <a class='product' id='${idProd}'>
        <button class = "buttonLike"><img src="/img/icons/like_empty.svg"></button>
            <img class = 'photo' src='/img/${items[card].imgUrl}'> 
            <div class='infoProd'><span class='name'>${items[card].name}</span></div>
        <div class='infoCard'>
            <div class='inform'>${items[card].orderInfo.inStock} left in stock</div>
            <div class='inform'>Price <b>${items[card].price}</b>$</div>
            <button class="buttonCard" id='myBtn${idProd}' >Add to card</button>
        </div>
        
        <div class="reviewsInfo">
            <div class=""> <b>${items[card].orderInfo.reviews}%</b> Positive reviews <span class="orders"><b>132</b></span></div>
            <div class=""> ${result}  <span class="orders">orders</span> </div>
        </div></div>
`;
}

document.getElementById('card').innerHTML = productCard + "</a>";




//Модалка
let cards = Array.from(document.getElementsByClassName("product")); 
cards.forEach(card => {
    card.addEventListener("click", evt => {
        ...
        <div class="modal-center">
            <div class="modal-title">${cardFromArray.name}</div>
            <div class="reviewsInfo">
                <div class=""> <b>${cardFromArray.orderInfo.reviews}%</b> Positive reviews <span class="orders"><b>132</b></span></div>
                <div class=""> ${result}  <span class="orders">orders</span> </div>
            </div>
        ...
        <div class="modal-right">
            ...
            if(cardFromArray.orderInfo.reviews > 50) {
                var result = "Above avarage";
            } else {
               var result = "Less than average";
            }
            // let ovr = document.getElementsByTagName('body').style.overflow('hidden');
...
//Поиск
let search = Array.from(document.getElementsByClassName("product")); 
search = window.document.getElementById('input').addEventListener('keyup', ({target}) => {
    const string = target.value.toLowerCase();
    let all = window.document.querySelectorAll('.product');
    all.forEach( el => {
        
    //let searchName = items.find(element => element.name == search.name);
        if(el.innerHTML.toLowerCase().indexOf(string)>-1){
            el.style.display = '';
        } else {
...
//Фильтры
function price() {
    document.getElementById("filtPrice").classList.toggle("block");
    document.querySelector(".priceFilt").style.backgroundColor = "#fffff";
}
function color() {
    document.getElementById("filtColor").classList.toggle("block");
}
function memory() {
    document.getElementById("filtMemory").classList.toggle("block");
}
function os() {
    document.getElementById("filtOs").classList.toggle("block");
}
function display() {
    document.getElementById("filtDisplay").classList.toggle("block");
}
...
function myFunction() {
  ...
  let prods = window.document.getElementById("products");
//   // create list of Li from items array
//   items.forEach((el, i) => {
//     const li = document.createElement("li");
//     li.setAttribute("id", i);
//     li.appendChild(document.createTextNode(el.elem));
//     li.className = el.color;
//     prods.appendChild(li);
//   });
//   //checkbox onclick handler
//   function toggleCheckbox(element) {
//     if (element.checked) {
//       colorFilter.push(element.id);
//     } else {
//       colorFilter.splice(colorFilter.indexOf(element.id), 1);
//     }
//     let listLi = Array.from(prods.getElementsByTagName("li"));
//     if (!colorFilter.length) {
//       listLi.forEach((el) => (el.style.display = ""));
//       return;
//     }
//     listLi.forEach((el) => {
//       //1 get all classes from element using classList
//       //2 get all classes from element using className
//       let classes = Array.from(el.classList);
//       el.style.display = colorFilter.find((element) => classes.includes(element))
//         ? ""
//         : "none";
//     });
//   }
